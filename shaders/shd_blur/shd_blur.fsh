...
void main() {
    ...
    vec4 diagonal_color = (
        ...
        texture2D(gm_BaseTexture, vec2(v_vTexcoord.x + texel.x, v_vTexcoord.y + texel.y))) / 4.; // southeast
    ...
    vec4 distant_color = (
        ...
        texture2D(gm_BaseTexture, vec2(v_vTexcoord.x + 2. * texel.x, v_vTexcoord.y + 2. * texel.y))) / 8.; // southeast
